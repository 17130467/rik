...
public class JPanelApp extends JPanel {
	...
	/*
	 * res, res1 переменные для операндов, используемых для расчета умножения,
	 * деления, сложения, вычитания
	 */
	double res = 0;
	double res1 = 0;
	...
	public JPanelApp() {
		...
		try {
			/*
			 * не используем компоновку элементов, расположение элементов
			 * зададим явно
			 */
			setLayout(null);
			...
			Ds.setModel(new SpinnerNumberModel(new Integer(0), new Integer(0),
					null, new Integer(1)));
			Ds.setBounds(10, 161, 104, 30);

			JSpinner CHILDs = new JSpinner();
			CHILDs.setModel(new SpinnerNumberModel(new Integer(0), new Integer(
			...
			b1.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg1) {
					...
					/*
					 * если res (первый операнд) равен 0, то конвертируем
					 * содержимое txt1 в Double и записываем в res
					 */
					if (res == 0) {
						res = Double.valueOf(txt1.getText());
					} else {
						/*
						 * если res (первый операнд) не равен 0, то конвертируем
						 * содержимое txt1 в Double и записываем в res1
						 */
			...
			/*
			 * аналогичным образом устанавливаем слушателей на следующие кнопки
			 * и реализуем реакцию на событие нажатие на кнопку
			 */
			...
			bPlus.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg1) {
					/*
					 * записываем в res конвертированное в Double содержимое
					 * txt1
					 */
					res = Double.valueOf(txt1.getText());
					/* отчищаем txt1 */
			...
			bRes.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					/*
					 * конвертируем res и res1 в Double, заносим в num, num1
					 * соответственно
					 */
					...
					/*
					 * вызываем у mc метод calc и передаем ему оператора и
					 * операнды
					 */
					/*
					 * результат работы метода ковертируем в String и заносим в
					 * переменныю strRes
					 */
			...
			TAXb.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent arg0) { // получаем
																// необходимые
																// для расчета
																// налога
																// значения из
																...
																// соответствующие
																// переменные
					double d = ((SpinnerNumberModel) Ds.getModel()).getNumber()
							.intValue();
					...
					tax tax = new tax();
					/*
					 * вызываем у tax метод calcTax и передаем ему необходимые
					 * значения
					 */
					/*
					 * результат работы метода конвертируем в String и заносим в
					 * переменную strRes
					 */
